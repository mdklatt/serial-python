...
these fields individually can be error-prone and tedious. In this case, the
format of each sensor field is the same so they can all be treated as a single
array. Each array element will have a value and a flag.

An `ArrayType` field must be initalized with the field definitions to use for
each array item. The position of the array itself is relative to the entire
input line, but the positions of the fields in each array item are relative to
each other.
...
object. With the sample data, the date and time fields can be treated as a
...
[format string][1].
    ...
    with open("data.txt", "r") as stream:
        ...
        for record in reader:
            print("total sensors: {0:d}".format(len(record["data"])))
            for sensor in record["data"]:
                print(record["timestamp"], sensor["value"], sensor["flag"])

### Default Values ###

During input, all fields in a record are assigned a value. If a field is blank
it is given a value of None. A default value can be assigned to a DataType;
this value will be used if the field is blank. The default value should be
appropriate to the DataType, *e.g.* an IntType field should not have a default
value of "N/A".
...
A filter can return a modified version of the input record or a different
record altogether.
    ...
    class LocalTime(object):
        ...
        def __init__(self, local=-6)  # default to UTC-6 (CST)
...
Writing data to a stream is (mostly) symmetric to reading. **Writers** use the
same field definitions as **Readers** with some additional requirements. A
**DataType** can be initialized with a format string. This is ignored by
**Readers**, but it is used by **Writers** to format a Python value as a string
(see the documenation for the format() function). Each **DataType** has a
default format, but for FixedWidthWriter fields a format string with the
appropriate field width must be specified.
...
A **Writer** expects a value to write for each of its fields for every data
record to be written. If an input record is missing a field the **Writer** will
use the default value for that field (None cannot be used as a default value
for output). If no default is defined an exception is raised. Fields in the
record that are not part of the data format are ignored.

Filters work for **Writers** like they do for **Readers**, but in reverse. The
filters defined for a **Writer** are applied to each record passed to the
write() method before the record is written to the stream.

With some minor modifications, the field definitions for reading the sample
data can be used for writing. In fact, the modified fields can still be used
for reading the data, so a **Reader** and a **Writer** can be defined for a
given data format using one set of field definitions.
...
in a class that inherits from the appropriate **Reader** or **Writer**, and
these classes can be bundled into a module for that format.
...
**Readers** and **Writers** are both initialized with stream arguments. A 
**Reader**'s input stream is any object that implements a `next()` method that
returns the next record from the stream as a single line of text. A 
**Writer**'s output stream is any object that implements a `write()` method to
write a line of text. A Python `file` object satisfies the requirements for
both types of streams. The `IStreamAdaptor` and `OStreamAdaptor` abstract
classes declare the required interfaces and can be used to create adaptors for
other types of streams, *e.g.* binary data.
...
streams. This is useful for parsing streams where the end of header data can
...
output, but filter can be used to convert to/from multiple Python types based
on the field value.
